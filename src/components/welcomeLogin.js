...
const WelcomeLogin = () => {
  ...
  const handleGuestLogin = () => {
    const existingGuestEmail = getCookie("guestEmail");
    ...
    if (existingGuestEmail) {
      ...
      guestName = getCookie("guestName"); 
      guestCheck = getCookie("guestCheck");
      console.log("Logging in with existing guest account: ", guestEmail);
    } 
    else {
      const timestamp = Date.now();
      const randomSuffix = Math.random().toString(36).substring(2, 6);
      guestEmail = `guest${timestamp}${randomSuffix}@example.com`;
      guestName = `Guest`;
      guestCheck = true;
  
      // Store the generated guest account in cookies
      setCookie("guestEmail", guestEmail, 30); // Expire after 7 days
      setCookie("guestName", guestName, 30);   // Expire after 7 days
      setCookie("guestCheck",guestCheck.toString(), 30);
  
      console.log("Created new guest account: ", guestEmail);
    }
  
    // Set guest user information in cookies or state
    setCookie("name", guestName, 30);
    setCookie("email", guestEmail, 30);
    setCookie("guestCheck",true, 30);
